...
valid_companies = set(
    pd.read_sql("SELECT DISTINCT company_id FROM financial_ratios", conn)["company_id"]
)
...
def safe_float(value):
    ...
    try:
        return float(
            str(value).split(":")[-1].replace("%", "").replace(",", "").strip()
        )
...
for _, row in df.iterrows():
    ...
    # PRO6
    profit_growth = safe_float(row["compounded_profit_growth"])
    ...
    # PRO8
    dividend_yield = safe_float(row["dividend_yield_pct"])
    ...
    # PRO11
    sales_growth = safe_float(row["compounded_sales_growth"])
    ...
    if row["debt_to_equity"] < 0.5:
        ...
        add_pro(
            company, "PRO12", "Low leverage supports long-term sustainable growth.", 80
        )
    ...
    if row["operating_profit_margin_pct"] < 10:
        ...
        add_con(company, "CON4", "Operating margin is relatively weak.", 75)
    ...
    if row["interest_coverage"] < 1.5:
        ...
        add_con(company, "CON6", "Low interest coverage increases financial risk.", 90)

    # CON7
    if row["dividend_payout_ratio_pct"] > 100:

        add_con(
            company, "CON7", "Dividend payout above 100% appears unsustainable.", 88
        )

    # CON8
    if row["debt_to_equity"] > 1.5:

        add_con(company, "CON8", "Debt levels are relatively high.", 82)

    # CON9
    if row["earnings_per_share"] < 5:

        add_con(
            company, "CON9", "Low earnings per share indicates weak profitability.", 75
        )

    # CON10
    if row["return_on_equity_pct"] < 10:

        add_con(company, "CON10", "Low return on capital efficiency.", 82)

    # CON11
    if row["total_debt_cr"] > 1000:

        add_con(company, "CON11", "High debt limits financial flexibility.", 78)

    # CON12
    if sales_growth is not None and sales_growth < 5:
...
pros_cons_df.drop_duplicates(subset=["company_id", "rule_id", "type"], inplace=True)
...
all_companies = df["company_id"].drop_duplicates()


for company in all_companies:

    company_rows = pros_cons_df[pros_cons_df["company_id"] == company]
...
pros_cons_df = pros_cons_df.sort_values(by=["company_id", "type", "rule_id"])

pros_cons_df.reset_index(drop=True, inplace=True)
...
print("Companies :", pros_cons_df["company_id"].nunique())

print("Total Records :", len(pros_cons_df))

print("Total Rules :", pros_cons_df["rule_id"].nunique())

print("Pro Rules :", pros_cons_df[pros_cons_df["type"] == "pro"]["rule_id"].nunique())

print("Con Rules :", pros_cons_df[pros_cons_df["type"] == "con"]["rule_id"].nunique())
